...
from triplet_loss import OnlineTripletLoss
...
margin = 0.3
triplet_criterion = OnlineTripletLoss(margin, RandomNegativeTripletSelector(margin))
...
def parse_args():
    ...
    parser.add_argument('--RGBD', type=str2bool, default=True)
    ...
    parser.add_argument(
        '--optimizer', type=str, choices=['sgd', 'adam', 'lars', 'rmsprop'])
    ...
    parser.add_argument(
        ...
        type=str,
        default='NYU',
        choices=['CIFAR10', 'CIFAR100', 'MNIST', 'FashionMNIST', 'KMNIST', 'NYU'])
    parser.add_argument('--num_workers', type=int, default=7)
    # standard data augmentation
    parser.add_argument('--use_random_crop', type=str2bool, default=True)
    parser.add_argument('--random_crop_padding', type=int, default=4)
    parser.add_argument('--use_horizontal_flip', type=str2bool, default=True)
    ...
    parser.add_argument('--cutout_prob', type=float, default=0.9)
    parser.add_argument('--cutout_inside', action='store_true', default=True)
    parser.add_argument('--dual_cutout_alpha', type=float, default=0.1)
    # random erasing configuration
    parser.add_argument(
        '--use_random_erasing', action='store_true', default=True)
    parser.add_argument('--random_erasing_prob', type=float, default=0.8)
    parser.add_argument(
        '--random_erasing_area_ratio_range', type=str, default='[0.02, 0.4]')
    parser.add_argument(
        '--random_erasing_min_aspect_ratio', type=float, default=0.2)
    ...
    parser.add_argument(
        '--use_label_smoothing', action='store_true', default=True)
...
def train(epoch, model, optimizer, scheduler, criterion, train_loader, config,
    ...
    start = time.time()
    for step, (data, depth, targets) in enumerate(train_loader):
        global_step += 1
        ...
        if 'ghost_batch_size' not in optim_config.keys():
            if data_config['use_dual_cutout']:
                ...
                if config['model_config']['arch'] == 'rgbd_alexnet_mm':
                    outputs, aux_rgb_out, aux_d_out, final_concat_out = model(data, depth)
                elif config['model_config']['arch'] == 'rgbd_resnet':
                    outputs, aux_rgb_out, aux_d_out, final_concat_out = model(data, depth)
                else:
                    #outputs = model(data, depth)
                    outputs = model(depth)
            if config['model_config']['arch'] == 'rgbd_alexnet_mm':
                loss = criterion(outputs, targets) + criterion(aux_rgb_out, targets) + criterion(aux_d_out, targets) \
                #+ 0.001 * triplet_criterion(final_concat_out, targets)
            elif config['model_config']['arch'] == 'rgbd_resnet':
                loss = criterion(outputs, targets) + criterion(aux_rgb_out, targets) + criterion(aux_d_out, targets) \
                #+ 0.001 * triplet_criterion(final_concat_out, targets)
            else:
            ...
            if data_config['use_dual_cutout']:
                ...
                depth_chunks = depth.chunk(n_split)
                if data_config['use_mixup']:
                    ...
                    target_chunks = targets.chunk(n_split)
                outputs = []
                for data_chunk, depth_chunk, target_chunk in zip(data_chunks, depth_chunks,
                                                    target_chunks):
                    output_chunk = model(data_chunk, depth_chunk)
...
def test(epoch, model, criterion, test_loader, model_arch, run_config, writer):
    ...
    cm.reset()
    with torch.no_grad():
        for step, (data, hha, targets) in enumerate(test_loader):
            ...
            hha = hha.to(device)
            targets = targets.to(device)
            if model_arch == 'rgbd_alexnet_mm':
                outputs, aux_rgb_out, aux_d_out, rgb_encode_out = model(data, hha)
                loss = criterion(outputs, targets) + criterion(aux_rgb_out, targets) + criterion(aux_d_out, targets) \
                #+ 0.001 * triplet_criterion(rgb_encode_out, targets)
            elif model_arch == 'rgbd_resnet':
                outputs, aux_rgb_out, aux_d_out, rgb_encode_out = model(data, hha)
                loss = criterion(outputs, targets) + criterion(aux_rgb_out, targets) + criterion(aux_d_out, targets) \
                #+ 0.001 * triplet_criterion(rgb_encode_out, targets)
            else:
                #outputs = model(data, hha)
                outputs = model(hha)
                loss = criterion(outputs, targets)
        ...
        cfm = cm.compute().float()
        all_prec = (cfm.diag()/cfm.sum(1))
        mP = all_prec.mean().cpu().data
        global best_mp
        if mP > best_mp:
            best_mp = mP
        print('Current mP {:.4f}, Best mP {:.4f}'. format(mP, best_mp))
...
def update_state(state, epoch, accuracy, mP, model, optimizer):
...
def main():
    ...
    if config['model_config']['arch'] == 'alexnet':
        model = model.load_pretrained_model(model)
    ...
    if run_config['test_first']:
        test(0, model, test_criterion, test_loader, model_arch, run_config, writer)
    ...
    for epoch, seed in zip(range(1, optim_config['epochs'] + 1), epoch_seeds):
        ...
        # test
        test_log = test(epoch, model, test_criterion, test_loader, model_arch, run_config,
                        writer)
        ...
        state = update_state(state, epoch, epoch_log['test']['accuracy'], epoch_log['test']['mp'],
                             model, optimizer)
